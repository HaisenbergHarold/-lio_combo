...
bool PoseManifold::Minus(const double *y, const double *x, double *y_minus_x) const {
    // Implement Minus for pose manifold
    ...
    p_diff = p_y - p_x;
    q_diff = Rotation::quaternion2vector(q_x.inverse() * q_y);

    return true;
}

bool PoseManifold::MinusJacobian(const double *x, double *jacobian) const {
    // MinusJacobian is the pseudo-inverse of PlusJacobian
    // For our PlusJacobian (7x6 with top 6x6 identity), the pseudo-inverse is 6x7 with left 6x6 identity
    Eigen::Map<Eigen::Matrix<double, 6, 7, Eigen::RowMajor>> jaco(jacobian);
    jaco.setZero();
    jaco.block<6, 6>(0, 0).setIdentity();
    return true;
}
