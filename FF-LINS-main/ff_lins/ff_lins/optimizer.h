...
public:
    ...
    // Covariance computation for the entire trajectory
    bool computeTrajectoryCovariance(ceres::Problem &problem, const deque<StateData> &statedatalist,
...
private:
    ...
    // 已记录协方差的关键帧时间戳（避免重复记录）
    std::set<double> recorded_covariance_timestamps_;

    // 协方差计算降频：每 N 帧计算一次
    int frame_count_{0};
    static constexpr int COVARIANCE_FRAME_INTERVAL = 10;
};
