...
bool Optimizer::optimization(const deque<double> &timelist, deque<StateData> &statedatalist,
    ...
    // 计算轨迹协方差（每 COVARIANCE_FRAME_INTERVAL 帧计算一次）
    // if (lidar_map_->isWindowFull() && !output_path_.empty()) {
    //     frame_count_++;
    //     if (frame_count_ % COVARIANCE_FRAME_INTERVAL == 0) {
    //         computeTrajectoryCovariance(problem, statedatalist, output_path_);
    //     }
    // }
...
LidarResiduals Optimizer::addLidarFactors(ceres::Problem &problem, bool isusekernel, const deque<double> &timelist,
    ...
    for (size_t i = 0; i < lidar_map_->keyframes().size() - 1; i++) {
        ...
        // points_lidar和features如何做到一一对应的: 提特征的时候就是每个points都提取特征的，但不一定每个点都对应到平面特征
        const auto &features = ref_frame->features();
        for (size_t k = 0; k < features.size(); k++) {
            const auto &feature = features[k];
            // 无效特征
...
bool Optimizer::computeTrajectoryCovariance(ceres::Problem &problem, const deque<StateData> &statedatalist,
                                            const string &output_path) {
    if (statedatalist.empty()) {
        LOGW << "Empty state data list, cannot compute covariance";
        return false;
    }

    // Get parameter blocks for all states
    vector<const double *> parameter_blocks;
    size_t num_states = statedatalist.size();

    // Pose parameters (7 each) and mix parameters (varies)
    int pose_param_size = Preintegration::numPoseParameter(); // should be 7
    int mix_param_size  = Preintegration::numMixParameter(preintegration_options_);

    LOGI << "Computing covariance for " << num_states << " states, "
         << "pose param size: " << pose_param_size << ", mix param size: " << mix_param_size;

    // Collect all parameter blocks - need to cast away const because Ceres expects const double*
    for (size_t i = 0; i < num_states; i++) {
        parameter_blocks.push_back(statedatalist[i].pose); // pose parameters
        parameter_blocks.push_back(statedatalist[i].mix);  // mix parameters
    }

    // Also include extrinsic parameters if they are being estimated
    if (optimize_estimate_lidar_extrinsic_ || optimize_estimate_lidar_td_) {
        parameter_blocks.push_back(extrinsic_b_l_); // 7 parameters for pose + 1 for td
        if (optimize_estimate_lidar_td_) {
            // td is part of extrinsic_b_l_ array at index 7
        }
    }
    // Compute covariance using Ceres
    ceres::Covariance::Options covariance_options;
    covariance_options.algorithm_type      = ceres::CovarianceAlgorithmType::SPARSE_QR;
    covariance_options.null_space_rank     = -1; // auto-detect
    ...
    if (!covariance.Compute(parameter_blocks, &problem)) {
        LOGW << "Failed to compute covariance matrix";
        return false;
    }

    LOGI << "Covariance computation successful";

    // Prepare output file for trajectory uncertainties
    string cov_file = output_path + "/trajectory_covariance.txt";

    // Check if file exists to decide whether to write header
    bool file_exists = std::ifstream(cov_file).good();
    std::ofstream ofs(cov_file, std::ios::app); // Append mode
    if (!ofs.is_open()) {
        LOGW << "Failed to open covariance output file: " << cov_file;
        return false;
    }

    // Write header only if file is newly created
    if (!file_exists) {
        ofs << "# Trajectory covariance (3σ standard deviations)" << std::endl;
        ofs << "# Format: timestamp(seconds) pos_x_3sigma(m) pos_y_3sigma(m) pos_z_3sigma(m) ";
        ofs << "quat_x_3sigma quat_y_3sigma quat_z_3sigma quat_w_3sigma ";
        ofs << "vel_x_3sigma(m/s) vel_y_3sigma(m/s) vel_z_3sigma(m/s)" << std::endl;
    }

    // For each state, extract covariance for its parameters
    int new_records = 0;
    for (size_t i = 0; i < num_states; i++) {
        double time = statedatalist[i].time;

        // Skip if already recorded
        if (recorded_covariance_timestamps_.find(time) != recorded_covariance_timestamps_.end()) {
            continue;
        }

        // Get covariance block for pose parameters (7x7)
        Eigen::MatrixXd pose_cov(pose_param_size, pose_param_size);
        if (!covariance.GetCovarianceBlock(statedatalist[i].pose, statedatalist[i].pose, pose_cov.data())) {
            LOGW << "Failed to get pose covariance for state " << i;
            continue;
        }

        // Get covariance block for mix parameters
        Eigen::MatrixXd mix_cov(mix_param_size, mix_param_size);
        if (!covariance.GetCovarianceBlock(statedatalist[i].mix, statedatalist[i].mix, mix_cov.data())) {
            LOGW << "Failed to get mix covariance for state " << i;
            continue;
        }

        // Extract 3σ standard deviations (sqrt of diagonal elements * 3)
        // Position (first 3 elements of pose parameters)
        double sigma_x = 3.0 * sqrt(pose_cov(0, 0));
        double sigma_y = 3.0 * sqrt(pose_cov(1, 1));
        double sigma_z = 3.0 * sqrt(pose_cov(2, 2));

        // Quaternion (next 4 elements)
        double sigma_qx = 3.0 * sqrt(pose_cov(3, 3));
        double sigma_qy = 3.0 * sqrt(pose_cov(4, 4));
        double sigma_qz = 3.0 * sqrt(pose_cov(5, 5));
        double sigma_qw = 3.0 * sqrt(pose_cov(6, 6));

        // Velocity (first 3 elements of mix parameters)
        double sigma_vx = 3.0 * sqrt(mix_cov(0, 0));
        double sigma_vy = 3.0 * sqrt(mix_cov(1, 1));
        double sigma_vz = 3.0 * sqrt(mix_cov(2, 2));

        // Write to file
        ofs << std::fixed << std::setprecision(9);
        ofs << time << " ";
        ofs << sigma_x << " " << sigma_y << " " << sigma_z << " ";
        ofs << sigma_qx << " " << sigma_qy << " " << sigma_qz << " " << sigma_qw << " ";
        ofs << sigma_vx << " " << sigma_vy << " " << sigma_vz << std::endl;
    }

    ofs.close();
    LOGI << "Trajectory covariance saved to: " << cov_file;

    // Also compute and save the full covariance matrix (optional, for debugging)
    if (num_states <= 10) { // Only for small problems
        string full_cov_file = output_path + "/full_covariance_matrix.txt";
        std::ofstream full_ofs(full_cov_file);
        if (full_ofs.is_open()) {
            // Compute full covariance matrix size
            size_t total_params = num_states * (pose_param_size + mix_param_size);
            if (optimize_estimate_lidar_extrinsic_ || optimize_estimate_lidar_td_) {
                total_params += 8; // extrinsic parameters
            }

            Eigen::MatrixXd full_cov(total_params, total_params);
            vector<const double *> all_blocks;

            // Re-collect all blocks in order
            for (size_t i = 0; i < num_states; i++) {
                all_blocks.push_back(statedatalist[i].pose);
                all_blocks.push_back(statedatalist[i].mix);
            }
            if (optimize_estimate_lidar_extrinsic_ || optimize_estimate_lidar_td_) {
                all_blocks.push_back(extrinsic_b_l_);
            }

            if (covariance.GetCovarianceMatrix(all_blocks, full_cov.data())) {
                full_ofs << "# Full covariance matrix (" << total_params << "x" << total_params << ")" << std::endl;
                full_ofs << full_cov.format(Eigen::IOFormat(15, 0, " ", "\n", "", "", "", ""));
                full_ofs.close();
                LOGI << "Full covariance matrix saved to: " << full_cov_file;
            }
        }
    }
