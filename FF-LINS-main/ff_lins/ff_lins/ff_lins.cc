...
LINS::LINS(const string &configfile, const string &outputpath, lidar::LidarViewer::Ptr lidar_viewer) {
    ...
    // cov_filesaver_        = FileSaver::create(outputpath + "/pose_covariance.txt", 7);
    ...
    // 设置优化器的输出路径
    optimizer_->setOutputPath(outputpath);

    // 耗时统计文件（10列：timestamp, total, merge, add_node, update_feature, lins_processing, optimization, redo_ins, add_cloud, remaining）
    timecost_filesaver_ = FileSaver::create(outputpath + "/timing_statistic.txt", 10);
...
void LINS::runFusion() {
    ...
    // 每关键帧全流程计时（跨LINS_NORMAL和optimization块共享）
    TimeCost frame_total_timecost;
    ...
    while (!is_finished_) {
        ...
        if (linsstate_ == LINS_INITIALIZING) {
            ...
            // 每关键帧全流程计时开始（在优化块中结束计时）
            frame_total_timecost.restart();

            int result = linsLidarProcessing(lidar_frame);
            // keyframe, extract feature
            if (result == 1) {

                // 合并非关键帧和当前帧
                TimeCost merge_timecost;
                lidar_map_->mergeNonKeyframes(lidar_nonkeyframes_, lidar_frame);
                double merge_cost = merge_timecost.costInMillisecond();
                ...
                // 加入新的雷达帧
                TimeCost add_node_timecost;
                addNewLidarFrameTimeNode(lidar_frame, true);
                double add_node_cost = add_node_timecost.costInMillisecond();

                // 更新点云特征
                TimeCost update_feature_timecost;
                lidar_map_->updateLidarFeaturesInMap();
                double update_feature_cost = update_feature_timecost.costInMillisecond();

                // linsLidarProcessing 耗时 = 当前累计 - merge - add_node - update_feature
                double cur_accumulated = frame_total_timecost.costInMillisecond();
                double lins_processing_cost = cur_accumulated - merge_cost - add_node_cost - update_feature_cost;
                if (lins_processing_cost < 0) lins_processing_cost = 0;

                // 保存部分计时结果，is_need_optimization 块中继续计时
                timing_record_.clear();
                timing_record_.push_back(lidar_frame->stamp());      // 0: timestamp
                timing_record_.push_back(0);                         // 1: total (placeholder)
                timing_record_.push_back(merge_cost);                // 2: merge_cost
                timing_record_.push_back(add_node_cost);             // 3: add_node_cost
                timing_record_.push_back(update_feature_cost);       // 4: update_feature_cost
                timing_record_.push_back(lins_processing_cost);      // 5: lins_processing_cost
        ...
        if (is_need_optimization) {
            ...
            // 优化求解（包含optimization和marginalization）
            TimeCost optimization_timecost;
            linsOptimizationProcessing();
            double optimization_cost = optimization_timecost.costInMillisecond();

            // 更新窗口内状态
            TimeCost redo_ins_timecost;
            ...
            double redo_ins_cost = redo_ins_timecost.costInMillisecond();

            // 可视化（同步部分：仅仅dispatch异步任务）
            TimeCost add_cloud_timecost;
            if (is_use_visualization_) {
            ...
            double add_cloud_cost = add_cloud_timecost.costInMillisecond();

            // 完成计时并写入文件（10列）
            {
                double total_cost = frame_total_timecost.costInMillisecond();
                vector<double> timing_data = timing_record_;
                timing_data[1] = total_cost;
                timing_data.push_back(optimization_cost);      // 6
                timing_data.push_back(redo_ins_cost);          // 7
                timing_data.push_back(add_cloud_cost);         // 8
                // remaining = total - (sum of all measured parts)
                double sum_measured = timing_data[2] + timing_data[3] + timing_data[4] + timing_data[5] +
                                      timing_data[6] + timing_data[7] + timing_data[8];
                double remaining = total_cost - sum_measured;
                if (remaining < 0) remaining = 0;
                timing_data.push_back(remaining);              // 9
                timecost_filesaver_->dump(timing_data);
                timecost_filesaver_->flush();
            }
        } else {
...
void LINS::writeNavResult(const IntegrationState &state) {
    ...
    // // covariance file (pose 3-sigma)
    // if (!preintegrationlist_.empty()) {
    //     auto preintegration = preintegrationlist_.back();
    //     const Eigen::MatrixXd& cov = preintegration->covariance();
    //     if (cov.rows() >= 15 && cov.cols() >= 15) {
    //         // Extract position and attitude covariance diagonal elements
    //         double sigma_x = 3.0 * sqrt(cov(0, 0));      // position x 3σ (m)
    //         double sigma_y = 3.0 * sqrt(cov(1, 1));      // position y 3σ (m)
    //         double sigma_z = 3.0 * sqrt(cov(2, 2));      // position z 3σ (m)
    //         double sigma_roll = 3.0 * sqrt(cov(6, 6));   // roll 3σ (rad)
    //         double sigma_pitch = 3.0 * sqrt(cov(7, 7));  // pitch 3σ (rad)
    //         double sigma_yaw = 3.0 * sqrt(cov(8, 8));    // yaw 3σ (rad)

    //         result.clear();
    //         result.push_back(time);
    //         result.push_back(sigma_x);
    //         result.push_back(sigma_y);
    //         result.push_back(sigma_z);
    //         result.push_back(sigma_roll);
    //         result.push_back(sigma_pitch);
    //         result.push_back(sigma_yaw);
    //         cov_filesaver_->dump(result);
    //         cov_filesaver_->flush();
    //     }
    // }
}
